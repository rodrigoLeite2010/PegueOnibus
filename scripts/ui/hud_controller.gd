...
@onready var _queue_holder: HBoxContainer = $Root/QueueHolder
@onready var _slots_holder: HBoxContainer = $Root/SlotsHolder
...
func _render_queue(queue: Array[String]) -> void:
	var visible_count: int = mini(queue.size(), 11)
	for index: int in range(visible_count):
		...
		passenger.custom_minimum_size = Vector2(36, 52)
		if index == 0:
			passenger.custom_minimum_size = Vector2(42, 58)
	...
	if _queue_more_label == null:
		...
		_queue_more_label.add_theme_font_size_override("font_size", 18)
		_queue_more_label.add_theme_color_override("font_color", Color("#555f73"))
...
func _render_slots(state: GameState) -> void:
	...
	for slot: WaitingSlotState in state.waiting_slots:
		_slots_holder.add_child(_build_slot_card(state, slot))

	# Duas vagas extras aparecem apenas como teaser visual, como no prototipo.
	for _index: int in range(2):
		_slots_holder.add_child(_build_add_slot_card())

func _build_slot_card(state: GameState, slot: WaitingSlotState) -> Control:
	var card := PanelContainer.new()
	card.custom_minimum_size = Vector2(80, 86)
	...
	if slot.is_empty():
		card.add_theme_stylebox_override("panel", _slot_style(Color(1, 1, 1, 0.035), Color("#f2f6ff")))
		var empty := Label.new()
		empty.text = ""
		empty.custom_minimum_size = Vector2(64, 62)
		box.add_child(empty)
		return card

	var vehicle: VehicleState = state.vehicles[slot.vehicle_id]
	var texture := VehicleVisualLibrary.get_for_color(vehicle.color_id)
	if texture != null and texture.sprite_texture != null:
		var preview := TextureRect.new()
		preview.texture = texture.sprite_texture
		...
		color_block.color = COLOR_MAP.get(vehicle.color_id, Color.WHITE)
		color_block.custom_minimum_size = Vector2(56, 42)
		box.add_child(color_block)

	if AccessibilitySettings.color_symbols_enabled:
		var symbol_label := Label.new()
		symbol_label.text = AccessibilitySettings.symbol_for(vehicle.color_id)
		symbol_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		symbol_label.add_theme_font_size_override("font_size", 15)
		symbol_label.add_theme_color_override("font_color", Color.WHITE)
		box.add_child(symbol_label)

	var capacity := Label.new()
	capacity.text = "%s/%s" % [vehicle.occupied_seats, vehicle.capacity]
	capacity.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	capacity.add_theme_font_size_override("font_size", 17)
	capacity.add_theme_color_override("font_color", Color.WHITE)
	box.add_child(capacity)

	var color: Color = COLOR_MAP.get(vehicle.color_id, Color.WHITE)
	card.add_theme_stylebox_override("panel", _slot_style(color.darkened(0.06), color.darkened(0.30)))
	return card

func _build_add_slot_card() -> Control:
	var card := PanelContainer.new()
	card.custom_minimum_size = Vector2(80, 86)
	card.rotation_degrees = -4.0
	card.mouse_filter = Control.MOUSE_FILTER_IGNORE
	card.add_theme_stylebox_override("panel", _slot_style(Color(1, 1, 1, 0.03), Color("#73ed89")))
	var plus := Label.new()
	plus.text = "+"
	plus.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	plus.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	plus.add_theme_font_size_override("font_size", 34)
	plus.add_theme_color_override("font_color", Color("#73ed89"))
	card.add_child(plus)
	return card
...
func _slot_style(fill: Color, border: Color) -> StyleBoxFlat:
