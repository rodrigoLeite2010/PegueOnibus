...
static func update_terminal_status(state: GameState, events: Array[GameEvent]) -> void:
	...
	if has_waiting_vehicle_for_front_color(state):
		return
	var slots_full: bool = state.are_all_slots_full()
	# Se ha vaga livre e algum veiculo do tabuleiro consegue sair agora, ainda
	# existe jogada valida (mesmo que esse veiculo nao seja da cor da frente).
	# Se os slots estao cheios, um veiculo saindo do tabuleiro nao teria onde
	# estacionar, entao isso nao conta como progresso.
	if not slots_full and has_any_valid_board_move(state):
		return
	# Nenhum veiculo em espera atende a cor da frente e nao ha jogada valida
	# no tabuleiro (ou por falta de vaga, ou porque tudo esta bloqueado):
	# nao existe transicao valida, e Game Over.
